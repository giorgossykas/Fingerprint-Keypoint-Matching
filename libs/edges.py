...
def edge_processing(image: np.array, threshold: int = 155):
    """
...
def sift_match(image_base: np.array, image_test: np.array):
    ...
    kp1, des1 = sift.detectAndCompute(image_base.astype('uint8'), None)
    kp2, des2 = sift.detectAndCompute(image_test.astype('uint8'), None)
    ###matcher = cv2.BFMatcher()
    ###matches = matcher.match(des1,des2)
    ###final_img = cv2.drawMatches(image_base, kp1, image_test, kp2, matches[:20],None)
    ...
    ##img_matches = np.empty((max(image_base.shape[0], image_test.shape[0]), image_base.shape[1]+image_test.shape[1], 3), dtype=np.uint8)
    ##cv2.drawMatches(image_base, kp1, image_test, kp2, good_matches, img_matches, flags=cv.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
    ##cv2.imshow('Good Matches', img_matches)
    ##cv2.waitKey(0)

    # Need to draw only good matches, so create a mask
    
    #Below commented red code is for single hash(#)
    ...
    img3 = cv2.drawMatchesKnn(image_base.astype('uint8'), kp1, image_test.astype('uint8'), kp2, matches, None, **draw_params)
