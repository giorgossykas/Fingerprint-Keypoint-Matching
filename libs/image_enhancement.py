...
class ProcessEnhance(FingerprintImageEnhancer):
    ...
    def binarize(self, img, clipLimit, tileGridSize):
        # Resize
        dim = 512
        ...
        nib = cl > nib_thr
        _, threshold = cv.threshold(cl, 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU)
    ...
    def process(self, img, clipLimit, tileGridSize):
        '''Runs two methods, first the preprocessing above and then the code for the enhancement'''
        niblack = self.binarize(img, clipLimit, tileGridSize)
